...
#include "../../capi/core_net/types.h"
#include "../../core/core_net/crypto_bls_ext.hpp"
 ...
 */
    inline void set_finalizers( const finalizer_policy& finalizer_policy ) {
        for (const auto& finalizer : finalizer_policy.finalizers)
            core_net::check(finalizer.public_key.size() == sizeof(bls_g1), "public key has a wrong size" );
        auto packed = core_net::pack(finalizer_policy);
