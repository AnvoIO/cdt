...
namespace core_net {
  ...
  class time_point_sec
  {
    public:
        ...
        operator time_point()const { return time_point( core_net::seconds( utc_seconds) ); }
        uint32_t sec_since_epoch()const { return utc_seconds; }

        /// @cond INTERNAL
        time_point_sec operator = ( const core_net::time_point& t )
        {
          utc_seconds = uint32_t(t.time_since_epoch().count() / 1000000ll);
          return *this;
        }
   ...
   class block_timestamp {
      public:
         ...
         block_timestamp next() const {
            core_net::check( std::numeric_limits<uint32_t>::max() - slot >= 1, "block timestamp overflow" );
