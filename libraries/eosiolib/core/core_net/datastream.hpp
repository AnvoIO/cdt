   ...
   public:
      ...
      */
      inline bool read( void* d, size_t s ) {
        core_net::check( size_t(_end - _pos) >= (size_t)s, "datastream attempted to read past the end" );
        memcpy( d, _pos, s );
        _pos += s;
        return true;
      }
      ...
      */
      inline bool write( const char* d, size_t s ) {
        core_net::check( _end - _pos >= (int32_t)s, "datastream attempted to write past the end" );
      ...
      */
      inline bool write( char d ) {
        core_net::check( _end - _pos >= 1, "datastream attempted to write past the end" );
        *_pos++ = d;
        return true;
      }
...
void deserialize(datastream<Stream>& ds, std::variant<Ts...>& var, int i) {
   if constexpr (I < std::variant_size_v<std::variant<Ts...>>) {
      ...
      core_net::check(false, "invalid variant index");
   }
}
...
datastream<Stream>& operator >> ( datastream<Stream>& ds, T (&v)[N] ) {
   unsigned_int s;
   ds >> s;
   core_net::check( N == s.value, "T[] size and unpacked size don't match");
   for( uint32_t i = 0; i < N; ++i )
      ds >> v[i];
   return ds;
}
