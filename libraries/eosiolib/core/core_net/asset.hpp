...
namespace core_net {
   ...
   struct asset {
      ...
      :amount(a),symbol{s}
      {
         core_net::check( is_amount_within_range(), "magnitude of asset amount must be less than 2^62" );
         core_net::check( symbol.is_valid(),        "invalid symbol name" );
      }
      ...
      asset& operator-=( const asset& a ) {
         core_net::check( a.symbol == symbol, "attempt to subtract asset with different symbol" );
         amount -= a.amount;
         core_net::check( -max_amount <= amount, "subtraction underflow" );
         core_net::check( amount <= max_amount,  "subtraction overflow" );
         return *this;
      }
       ...
       */
      asset& operator+=( const asset& a ) {
         core_net::check( a.symbol == symbol, "attempt to add asset with different symbol" );
         amount += a.amount;
         core_net::check( -max_amount <= amount, "addition underflow" );
         core_net::check( amount <= max_amount,  "addition overflow" );
         return *this;
      }
      ...
      asset& operator*=( int64_t a ) {
         ...
         core_net::check( tmp <= max_amount, "multiplication overflow" );
         core_net::check( tmp >= -max_amount, "multiplication underflow" );
         amount = (int64_t)tmp;
         return *this;
      }
       ...
       */
      asset& operator/=( int64_t a ) {
         core_net::check( a != 0, "divide by zero" );
         core_net::check( !(amount == std::numeric_limits<int64_t>::min() && a == -1), "signed division overflow" );
         amount /= a;
         return *this;
      }
      ...
      friend int64_t operator/( const asset& a, const asset& b ) {
         core_net::check( b.amount != 0, "divide by zero" );
         core_net::check( a.symbol == b.symbol, "attempt to divide assets with different symbol" );
      ...
      friend bool operator==( const asset& a, const asset& b ) {
         core_net::check( a.symbol == b.symbol, "comparison of assets with different symbols is not allowed" );
         return a.amount == b.amount;
      }
   ...
   */
   struct extended_asset {
      ...
      friend extended_asset operator - ( const extended_asset& a, const extended_asset& b ) {
         core_net::check( a.contract == b.contract, "type mismatch" );
