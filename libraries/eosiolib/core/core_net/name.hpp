...
namespace core_net {
   ...
   public:
      ...
      {
         if( str.size() > 13 ) {
            core_net::check( false, "string is too long to be a valid name" );
         }
         if( str.empty() ) {
            return;
         }
         ...
         if( str.size() == 13 ) {
            ...
            if( v > 0x0Full ) {
               core_net::check(false, "thirteenth character in name cannot be a letter that comes after j");
            }
            value |= v;
         }
      }
      ...
      static constexpr uint8_t char_to_value( char c ) {
         if( c == '.')
            ...
            core_net::check( false, "character is not in allowed character set for names" );
...
inline constexpr core_net::name operator""_n() {
   constexpr auto x = core_net::name{std::string_view{core_net::detail::to_const_char_arr<Str...>::value, sizeof...(Str)}};
   return x;
}
