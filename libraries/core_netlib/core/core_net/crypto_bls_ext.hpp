...
namespace detail {
    ...
    std::string bls_type_to_base64url(const T& g1) {
        ...
        return Prefix + core_net::base64url_encode({g1_with_checksum.data(), g1_with_checksum.size()});
    }

    template<typename T, const std::string& Prefix>
    T bls_base64url_to_type(const char* data, size_t size) {
        core_net::check(size > Prefix.size(), "encoded base64 key is too short");
        core_net::check(0 == memcmp(data, Prefix.data(), Prefix.size()), "base64 encoded type must begin from corresponding prefix");

        std::string decoded = core_net::base64url_decode({data+Prefix.size(), size - Prefix.size()});
        T ret;
        core_net::check(decoded.size() == ret.size() + bls_checksum_size, "decoded size " + std::to_string(decoded.size()) + 
        ...
        core_net::check(0 == memcmp(&*it, csum.data(), bls_checksum_size), "checksum of structure doesn't match");
