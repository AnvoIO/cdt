...
namespace core_net {
   ...
   /**
    *  Serialize an core_net::webauthn_public_key into a stream
    *
    *  @ingroup public_key
   ...
   inline DataStream& operator<<(DataStream& ds, const core_net::webauthn_public_key& pubkey) {
      ds << pubkey.key << pubkey.user_presence << pubkey.rpid;
      return ds;
   }

   /**
    *  Deserialize an core_net::webauthn_public_key from a stream
    *
    *  @ingroup public_key
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, core_net::webauthn_public_key& pubkey) {
      ds >> pubkey.key >> pubkey.user_presence >> pubkey.rpid;
      return ds;
   }
   ...
   /**
    *  Serialize an core_net::webauthn_signature into a stream
    *
    *  @param ds - The stream to write
    ...
    */
   template<typename DataStream>
   inline DataStream& operator<<(DataStream& ds, const core_net::webauthn_signature& sig) {
      ds << sig.compact_signature << sig.auth_data << sig.client_json;
      return ds;
   }

   /**
    *  Deserialize an core_net::webauthn_signature from a stream
    *
    *  @param ds - The stream to read
    ...
    */
   template<typename DataStream>
   inline DataStream& operator>>(DataStream& ds, core_net::webauthn_signature& sig) {
      ds >> sig.compact_signature >> sig.auth_data >> sig.client_json;
      return ds;
   }
    ...
    *  @param data - Data you want to hash
    *  @param length - Data length
    *  @return core_net::checksum160 - Computed digest
    */
    ...
    *  @param data - Data you want to hash
    *  @param length - Data length
    *  @return core_net::checksum512 - Computed digest
    */
    ...
    *  @param sig - Signature
    *  @return core_net::public_key - Recovered public key
    */
