...
#include "../../contracts/core_net/action.hpp"
#include "../../core/core_net/name.hpp"
#include "../../core/core_net/serialize.hpp"
#include "../../core/core_net/fixed_bytes.hpp"
...
namespace _multi_index_detail {
   ...
   WRAP_SECONDARY_ARRAY_TYPE(idx256, core_net::fixed_bytes<32>)
   template<>
   struct secondary_key_traits<core_net::fixed_bytes<32>> {
      static constexpr core_net::fixed_bytes<32> true_lowest() { return core_net::fixed_bytes<32>(); }
   };

   template<typename PK>
   inline uint64_t to_raw_key(PK pk) { return pk; }
   inline uint64_t to_raw_key(core_net::name pk) { return pk.value; }
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            constexpr static bool validate_index_name( core_net::name n ) {
               return n.value != 0 && n != core_net::name("primary"); // Primary is a reserve index name.
            }
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     core_net::check( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        core_net::check( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        core_net::check( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code().value, _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        core_net::check( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               core_net::check( lb != cend(), error_msg );
               core_net::check( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               core_net::check( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               core_net::check( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, core_net::name payer, Lambda&& updater ) {
               core_net::check( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            template<typename Lambda>
            void modify( const T& obj, core_net::name payer, Lambda&& updater ) {
               _multidx->modify( obj, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               core_net::check( itr != cend(), "cannot pass end iterator to erase" );
            ...
            core_net::name get_code()const  { return _multidx->get_code(); }
      ...
      class make_index_tuple {
         ...
         static constexpr auto get_type(std::index_sequence<Seq...>) {
            return std::make_tuple(std::make_tuple(index<core_net::name::raw(static_cast<uint64_t>(Values::index_name)),
                                                         typename Values::secondary_extractor_type,
                                                         intc<Seq>::e::value, const_index>{},
                                                   index<core_net::name::raw(static_cast<uint64_t>(Values::index_name)),
                                                         typename Values::secondary_extractor_type,
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         core_net::check( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       * Notes
       * The `core_net::multi_index` template has template parameters `<name::raw TableName, typename T, typename... Indices>`, where:
       * - `TableName` is the name of the table, maximum 12 characters long, characters in the name from the set of lowercase letters, digits 1 to 5, and the "." (period) character and is converted to a core_net::raw - which wraps uint64_t;
       ...
       * - It is recommended to use the core_net::const_mem_fun template
       *
       * Example:
       *
       * @code
       ...
       *     typedef core_net::multi_index< "address"_n, address > address_index;
       *     void myaction() {
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               core_net::check( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = internal_use_do_not_use::db_previous_i64( ei , &prev_pk );
               core_net::check( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = internal_use_do_not_use::db_previous_i64( _item->__primary_itr, &prev_pk );
               core_net::check( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *       core_net::check(itr->account_name == name("dan"), "Lock arf, Incorrect Last Record ");
       *       itr++;
       ...
       *       itr--;
       *       core_net::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Record ");
       *       itr--;
       ...
       *       core_net::check(itr->account_name == name("brendan"), "Lock arf, Incorrect First Lower Bound Record ");
       *       itr++;
       *       core_net::check(itr->account_name == name("dan"), "Lock arf, Incorrect Second Lower Bound Record");
       *       itr++;
       ...
       *       core_net::check(itr->account_name == name("dan"), "Lock arf, Incorrect First Upper Bound Record ");
       *       itr++;
       *       core_net::check(itr == zip_index.end(), "Lock arf, Incorrect End of Iterator");
       *     }
       * }
      ...
      uint64_t available_primary_key()const {
         ...
         core_net::check( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *     typedef core_net::multi_index< name("address"), address, indexed_by< name("zip"), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *     void myaction() {
       *       // create reference to address_index  - see emplace example below
       *       // add dan account to table           - see emplace example below
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         core_net::check( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( name payer, Lambda&& constructor ) {
         ...
         core_net::check( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *
       *       auto itr = addresses.find("dan"_n);
       *       core_net::check(itr != addresses.end(), "Address for account not found");
      ...
      void modify( const_iterator itr, name payer, Lambda&& updater ) {
         core_net::check( itr != end(), "cannot pass end iterator to modify" );
      ...
      void modify( const T& obj, name payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         core_net::check( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         core_net::check( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         core_net::check( pk == _multi_index_detail::to_raw_key(obj.primary_key()), "updater cannot change primary key when modifying an object" );
      ...
      const_iterator require_find( PK primary, const char* error_msg = "unable to find key" )const {
         ...
         uint64_t primary_int = _multi_index_detail::to_raw_key(primary);
         auto itr = internal_use_do_not_use::db_find_i64( _code.value, _scope, static_cast<uint64_t>(TableName), primary_int );
         core_net::check( itr >= 0,  error_msg );
       ...
       */
      const_iterator erase( const_iterator itr ) {
         core_net::check( itr != end(), "cannot pass end iterator to erase" );
       ...
       *     void myaction() {
       *       auto itr = addresses.find("dan"_n);
       *       core_net::check(itr != addresses.end(), "Record is not found");
       *       addresses.erase(*itr);
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         core_net::check( objitem.__idx == this, "object passed to erase is not in multi_index" );
         core_net::check( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         core_net::check( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );
