...
void LinkerDriver::linkerMain(ArrayRef<const char *> argsArr) {
  ...
  if (!config->relocatable && !config->entry.empty()) {
    ...
    } else if (config->entry == "apply") {
      // Create synthetic apply function — dispatch body generated by Writer
      WasmSignature applySig;
      applySig.Params = {ValType::I64, ValType::I64, ValType::I64};
      auto *synFunc = make<SyntheticFunction>(applySig, "apply");
      // Set a default empty body (just "end" opcode) — Writer will replace it
      const uint8_t defaultBody[] = {0 /*num locals*/, 0x0b /*end*/};
      synFunc->setBody(defaultBody);
      auto *applyFunc = symtab->addSyntheticFunction(
          "apply", WASM_SYMBOL_VISIBILITY_DEFAULT | WASM_SYMBOL_EXPORTED,
          synFunc);
      applyFunc->markLive();
      entrySym = applyFunc;
      entrySym->forceExport = true;
  ...
  // survive garbage collection. These weak symbols are referenced by
