...
// CDT WASM opcodes not in LLVM's standard set
static constexpr int OPCODE_IF         = 0x4;
static constexpr int OPCODE_ELSE       = 0x5;
static constexpr int OPCODE_GET_LOCAL  = WASM_OPCODE_LOCAL_GET;   // 0x20
static constexpr int OPCODE_SET_LOCAL  = WASM_OPCODE_LOCAL_SET;   // 0x21
static constexpr int OPCODE_GET_GLOBAL = WASM_OPCODE_GLOBAL_GET;  // 0x23
static constexpr int OPCODE_SET_GLOBAL = WASM_OPCODE_GLOBAL_SET;  // 0x24
...
// Aliases for opcodes that already exist in LLVM 16's WASM_OPCODE_* enum
static constexpr int OPCODE_CALL       = WASM_OPCODE_CALL;
static constexpr int OPCODE_END        = WASM_OPCODE_END;
static constexpr int OPCODE_RETURN     = WASM_OPCODE_RETURN;
static constexpr int OPCODE_I32_CONST  = WASM_OPCODE_I32_CONST;
static constexpr int OPCODE_I32_ADD    = WASM_OPCODE_I32_ADD;
static constexpr int OPCODE_I32_TYPE   = 0x7f;
static constexpr int OPCODE_I64_CONST  = WASM_OPCODE_I64_CONST;
static constexpr int OPCODE_I64_STORE  = WASM_OPCODE_I64_STORE;
static constexpr int OPCODE_I64_ADD    = WASM_OPCODE_I64_ADD;
...
// Generate the apply() WASM entry point dispatcher.
// This dispatches actions and notification handlers based on the
// receiver, code, and action parameters.
void Writer::createDispatchFunction() {
   ...
   // If apply is a real function (not synthetic), it was compiled from source
   // (e.g., generated by cdt-cpp codegen or user-defined). Don't overwrite it.
   if (!isa<SyntheticFunction>(applyFunc->function))
      return;
   ...
   auto create_notify_dispatch = [&](raw_string_ostream& OS) {
      ...
      if (not_cnt > 0) {
         ...
         for (auto const& notif0 : notify_handlers) {
            ...
            for (size_t i=0; i < notif0.second.size(); i++)
               writeU8(OS, OPCODE_END, "END");
   ...
   std::string bodyContent;
   {
      ...
      if (WasmSym::callCtors && WasmSym::callCtors->isLive()) {
         uint32_t ctors_idx = WasmSym::callCtors->getFunctionIndex();
         writeU8(OS, OPCODE_CALL, "CALL");
         writeUleb128(OS, ctors_idx, "__wasm_call_ctors");
      }

      // create the pre_dispatch function call
      auto pre_sym = dyn_cast_or_null<FunctionSymbol>(symtab->find("pre_dispatch"));
      if (pre_sym) {
      ...
      writeU8(OS, OPCODE_END, "END");
      auto dtors_sym = dyn_cast_or_null<FunctionSymbol>(symtab->find("__cxa_finalize"));
      if (dtors_sym) {
...
void Writer::createCallDispatchFunction() {
   ...
   auto create_if = [&](raw_string_ostream& os, std::string call_name, bool& need_else) {
      ...
      writeUleb128(os, 3, "data");
      writeU8(os, OPCODE_CALL, "CALL");
      writeUleb128(os, get_header_idx, "get_header_idx");
      ...
      writeU8(os, OPCODE_IF, "IF version != 0");
      writeU8(os, 0x40, "none");
      writeU8(os, OPCODE_I64_CONST, "I64.CONST");
      ...
      writeU8(os, OPCODE_I32_CONST, "I32 CONST");
      writeUleb128(os, 8, "8");
      writeU8(os, OPCODE_I32_ADD, "i32.add");
   ...
   std::string bodyContent;
   {
      ...
      // Declare two i32 locals, whose indices starting from 3,
