...
public:
   ...
   void hstretvaltst() {
      ...
      auto expected_size = core_net::call("callee"_n, 0, data.data(), data.size());
      core_net::check(expected_size >= 0, "call did not return a positive value");

      std::vector<char> return_value;
      return_value.resize(expected_size);
      auto actual_size = core_net::get_call_return_value(return_value.data(), return_value.size());
      core_net::check(actual_size == expected_size, "actual_size not equal to expected_size");
      core_net::check(core_net::unpack<uint32_t>(return_value) == 10u, "return value not 10");  // return_ten always returns 10
   }
   ...
   void hstoneprmtst() {
      ...
      const std::vector<char> data{ core_net::pack(std::make_tuple(header, 5)) };
      auto expected_size = core_net::call("callee"_n, 0, data.data(), data.size());
      core_net::check(expected_size >= 0, "call did not return a positive value");
      ...
      auto actual_size = core_net::get_call_return_value(return_value.data(), return_value.size());
      core_net::check(actual_size == expected_size, "actual_size not equal to expected_size");
      core_net::check(core_net::unpack<uint32_t>(return_value) == 5u, "return value not 5");  // echo_input returns back the same value of parameter
   }
   ...
   void hstmulprmtst() {
      ...
      auto actual_size = core_net::get_call_return_value(return_value.data(), return_value.size());
      core_net::check(actual_size == expected_size, "actual_size not equal to expected_size");
      core_net::check(core_net::unpack<uint32_t>(return_value) == 60u, "sum of 10, 20, and 30 not 60");  // sum returns the sum of the 3 arguments
   }
   ...
   void structtest() {
      ...
      core_net::check(output.a == input.a, "field a in output is not equal to a in input");
      core_net::check(output.b == input.b, "field b in output is not equal to b in input");
   }
   ...
   void structinttst() {
      ...
      core_net::check(output.a == m * input1.a + input2.c, "field a of output is not correct");
   ...
   void hdrvaltest() {
      ...
      const std::vector<char> unkwn_func_data{ core_net::pack(unkwn_func_header) }; // unknwnfunc is not in "callee"_i contract
      auto status = core_net::call("callee"_n, 0, unkwn_func_data.data(), unkwn_func_data.size());
      core_net::check(status == -10001, "call did not return -10001 for unknown function");
      ...
      const std::vector<char> bad_version_data{ core_net::pack(bad_version_header) };
      status = core_net::call("callee"_n, 0, bad_version_data.data(), bad_version_data.size());
      core_net::check(status == -10000, "call did not return -10000 for invalid version");
   }
