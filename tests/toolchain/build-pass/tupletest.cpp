 ...
 *
 * Important Remarks:
 *      1) The input of a std::tuple<T0,T1,T2> via core-cli uses [ele0, ele1, ele2], it is the same as the input of a vector/set
...
class [[core_net::contract("tupletest")]] tupletest : public core_net::contract {
    ...
    public:
        ...
        {
            SETCONTAINERVAL(t);
            core_net::print("tuple<uint16_t, string> stored successfully");
        }

        /*Examples:
         *  core-cli --verbose push action tupletest prntt '["alice"]' -p alice@active
        ...
        {
            PRNTCHECK();
            core_net::print("elements of stored tuple t:", std::get<0>(iter->t), ", ", std::get<1>(iter->t));
        }
        ...
        {
            SETCONTAINERVAL(vt);
            core_net::print("type defined vector<tup_uint16>  or vector<tuple<Ts...> stored successfully!");

        }

        /*Example:
         *  core-cli --verbose push action tupletest  prntvt '["alice"]' -p alice@active
        ...
        void prntvt(name user)
        {
            ...
            PRNTCHECK();
            core_net::print("stored vector<tuple<Ts...> vals:\n");
            for (int i=0 ; i < iter->vt.size(); i++)
            {
                core_net::print(std::get<0>(iter->vt[i]), " ", std::get<1>(iter->vt[i]));
                core_net::print("\n");
            }
        }
        ...
        {
            SETCONTAINERVAL(stt);
            core_net::print("type defined set<tup_uint16> or set<tuple<Ts...> stored successfully!");

        }

        /*Example:
         *  core-cli --verbose push action tupletest   prntstt '["alice"]' -p alice@active
        ...
        void prntstt(name user)
        {
            ...
            PRNTCHECK();
            core_net::print("stored set<tuple<Ts...> vals:\n");
            for (auto it1=iter->stt.begin(); it1!= iter->stt.end(); it1++)
            {
                core_net::print(std::get<0>(*it1), " ", std::get<1>(*it1));
                core_net::print("\n");
            }
        }
        ...
        /*Examples:
         *  core-cli --verbose push action tupletest setot '["bob", null]' -p bob@active
         *
        ...
        {
            SETCONTAINERVAL(ot);
            core_net::print("type defined optional<tuple<Ts...> > stored successfully!");
        }

        /*Examples:
         *  core-cli --verbose push action tupletest  prntot '["bob"]' -p bob@active
         *      output: >> stored optional<tup_uint16>  vals:
         *              >> NULL or no value
         *
         *  core-cli --verbose push action tupletest  prntot '["alice"]' -p alice@active
         *      output: >> stored optional<tup_uint16>  vals:
         *              >> 1001 2001
         */
        [[core_net::action]]
        ...
        {
            PRNTCHECK();
            core_net::print("stored optional<tup_uint16>  vals:\n");
            if (iter->ot)
            {
                core_net::print(std::get<0>(iter->ot.value()), " ", std::get<1>(iter->ot.value()));
            }
            else
        ...
        {
            SETCONTAINERVAL(mt);
            core_net::print("type defined map<K, tuple<Ts...> > stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prntmt '["alice"]' -p alice@active
        ...
        {
            PRNTCHECK();
            core_net::print("stored map<uint16_t, tup_uint16>: size=", iter->mt.size()," content:\n");
            for (auto it2 = iter->mt.begin(); it2 != iter->mt.end(); ++it2)
            {
                    core_net::print(it2->first, ":", "vals ");
                    core_net::print(std::get<0>(it2->second), " ", std::get<1>(it2->second));
                    core_net::print("\n");
            }
        }
        ...
        {
            SETCONTAINERVAL(pt);
            core_net::print("type-defined pair-tuple pair<uint32_t, tup_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prntpt '["alice"]' -p alice@active
        ...
        {
            PRNTCHECK();
            core_net::print("content of stored pair<uint32_t, tup_uint16>: first=", iter->pt.first, "\nsecond=");
            core_net::print(std::get<0>(iter->pt.second), " ", std::get<1>(iter->pt.second));
        }
        ...
        {
            SETCONTAINERVAL(tt);
            core_net::print("type defined tuple<tuple<Ts...> > stored successfully!");

        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttt '["alice"]' -p alice@active
        ...
        void prnttt(name user)
        {
            ...
            PRNTCHECK();
            core_net::print("stored tuple<tup_uint16, tup_uint16,  tup_uint16> vals:\n");
            const auto& ele0 = std::get<0>(iter->tt);
            const auto& ele1 = std::get<1>(iter->tt);
            const auto& ele2 = std::get<2>(iter->tt);

            core_net::print(std::get<0>(ele0), " ", std::get<1>(ele0), "\n");
            core_net::print(std::get<0>(ele1), " ", std::get<1>(ele1), "\n");
            core_net::print(std::get<0>(ele2), " ", std::get<1>(ele2), "\n");
        }
        ...
        {
            SETCONTAINERVAL(tv);
            core_net::print("type defined tuple<uint16_t, vec_uint16, vec_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttv '["alice"]' -p alice@active
        ...
        void prnttv(name user)
        {
            ...
            core_net::print("ele0: ", std::get<0>(iter->tv),"\n");
            core_net::print("ele1: "); printVec(std::get<1>(iter->tv));
            core_net::print("ele2: "); printVec(std::get<2>(iter->tv));
        }
        ...
        {
            SETCONTAINERVAL(tst);
            core_net::print("type defined tuple<uint16_t, set_uint16, set_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttst '["alice"]' -p alice@active
        ...
        void prnttst(name user)
        {
            ...
            core_net::print("stored tuple<uint16_t, set_uint16, set_uint16> vals:\n");
            ...
            core_net::print("ele0: ", std::get<0>(iter->tst),"\n");
            core_net::print("ele1: "); printSet(std::get<1>(iter->tst));
            core_net::print("ele2: "); printSet(std::get<2>(iter->tst));
        }

        /*Examples:
         *  core-cli --verbose push action tupletest  setto '["alice", [100, null, 200, null, 300]]' -p alice@active
         *
        ...
        {
            SETCONTAINERVAL(to);
            core_net::print("type defined tuple<op_uint16, op_uint16, op_uint16,op_uint16,op_uint16> stored successfully!");
        }

        /*Examples:
         *  core-cli --verbose push action tupletest  prntto '["alice"]' -p alice@active
         *      output: >> stored tuple<op_uint16, op_uint16, op_uint16,op_uint16,op_uint16> vals:
         *              >> 100 NULL 200 NULL 300
         *
         *  core-cli --verbose push action tupletest  prntto '["bob"]' -p bob@active
         *      output: >> stored tuple<op_uint16, op_uint16, op_uint16,op_uint16,op_uint16> vals:
        ...
        {
            PRNTCHECK();
            core_net::print("stored tuple<op_uint16, op_uint16, op_uint16,op_uint16,op_uint16> vals:\n");
            auto printOptional = [](op_uint16 op) {
                if (op)
                    core_net::print(op.value(),  " ");
                else
                    core_net::print("NULL", " ");
            };
        ...
        {
            SETCONTAINERVAL(tm);
            core_net::print("type defined tuple<uint16_t, mp_uint16, mp_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttm '["alice"]' -p alice@active
        ...
        void prnttm(name user)
        {
            ...
            core_net::print("stored tuple<uint16_t, mp_uint16, mp_uint16> vals:\n");

            auto printMap = [](mp_uint16 m) {
                for (auto it = m.begin(); it !=m.end(); it++ )
                    core_net::print(it->first, ":", it->second, " ");
                core_net::print("\n");
            };
            core_net::print("ele0: ", std::get<0>(iter->tm),"\n");
            core_net::print("ele1: "); printMap(std::get<1>(iter->tm));
            core_net::print("ele2: "); printMap(std::get<2>(iter->tm));
        }
        ...
        {
            SETCONTAINERVAL(tp);
            core_net::print("type defined tuple<uint16_t, pr_uint16, pr_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttp '["alice"]' -p alice@active
        ...
        void prnttp(name user)
        {
            ...
            core_net::print("stored tuple<uint16_t, pr_unit16, pr_unit16> vals:\n");

            auto printPair = [](pr_uint16 p) {
                core_net::print(p.first, ":", p.second);
                core_net::print("\n");
            };
            core_net::print("ele0: ", std::get<0>(iter->tp),"\n");
            core_net::print("ele1: "); printPair(std::get<1>(iter->tp));
            core_net::print("ele2: "); printPair(std::get<2>(iter->tp));
        }
        ...
        void settmisc(name user, const tuple<string, vec_uint16, pr_uint16>  & tmisc)
        {
            ...
            core_net::print("type defined tuple<string, vec_uint16, pr_uint16> stored successfully!");
        }

        /*Example:
         *  core-cli --verbose push action tupletest  prnttmisc '["alice"]' -p alice@active
        ...
        void prnttmisc(name user)
        {
            ...
            core_net::print("stored tuple<string, vec_uint16, pr_uint16> vals:\n");
            ...
            core_net::print("ele0: ", std::get<0>(iter->tmisc),"\n");
            core_net::print("ele1: "); printVec(std::get<1>(iter->tmisc));
            core_net::print("ele2: "); printPair(std::get<2>(iter->tmisc));
        }
};