...
#if defined(_WIN32)
#include "win.hpp"
#define __CORE_NET_OS__ sys::_win

#elif defined(__linux__) || defined(__CYGWIN__)
#include "linux.hpp"
#define __CORE_NET_OS__ sys::_linux

#elif defined(__APPLE__)
#include "osx.hpp"
#define __CORE_NET_OS__ sys::_osx

#elif defined(__DragonFly__) || defined(__FreeBSD__) || \
      defined(__FreeBSD_kernel__) || defined(__NetBSD__)
#include "bsd.hpp"
#define __CORE_NET_OS__ sys::_bsd
...
struct whereami {
   static inline int getExecutablePath(char* out, int capacity, int* dirname_length) {
      return _getExecutablePath<__CORE_NET_OS__>(out, capacity, dirname_length);
   }

   static inline int getModulePath(char* out, int capacity, int* dirname_length) {
      return _getModulePath<__CORE_NET_OS__>(out, capacity, dirname_length);
   }
