...
Add a second data member `secondary`, of type `core_net::name`, to the `test_table` data structure that defines the `testtab` data.
...
In the `test_table_t` type definition, add the definition of the secondary index with the use of the `core_net::indexed_by` template. `core_net::index_by` needs two parameters: the name of the index, `"secid"_n`, and a function call operator which extracts the value from the secondary data member as an index key. The function call operator is defined with the use of `core_net::const_mem_fun` template which receives two parameters: the data structure `test_table` and the reference to the getter function member `by_secondary`.
